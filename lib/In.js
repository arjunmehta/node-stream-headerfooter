...
In.prototype._transform = function(chunk, enc, cb) {
    ...
    parseStreamTags(this);
    cb();
};

function parseStreamTags(stream) {

    var delimiter = stream.delimiter,
        idx = buffertools.indexOf(stream.buffer, delimiter);

    if (idx !== -1) {
        handleTag(stream, delimiter, idx);
    } else {
        testPartial(stream, delimiter);
    }
}

function handleTag(stream, delimiter, idx) {
...
function testPartial(stream, delimiter) {
